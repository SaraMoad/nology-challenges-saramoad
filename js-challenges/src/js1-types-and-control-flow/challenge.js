...
export const getIsValidOnNameTag = () => {
  //write code here
  if (
    nameTagOption.charCodeAt(0) > 64 &&
    nameTagOption.charCodeAt(0) < 91 &&
...
export const getHasUppercaseLetters = () => {
  // Write your code here
  const pattern = /([A-Z])/g;
  const result = pattern.test(stringWithUppercaseLetters);
  return result;
};
...
export const convertPascalCaseToSnakeCase = () => {
  ...
  for (let i = 0; i < upperCaseLetters.length; i++) {
    let firstCharacter = pascalCaseVariableName.indexOf(upperCaseLetters[i]);
    let finalCharacter = pascalCaseVariableName.indexOf(
      upperCaseLetters[i + 1]
    );
    if (i === upperCaseLetters.length - 1) {
      finalCharacter = pascalCaseVariableName.length;
    }
    let word = pascalCaseVariableName.slice(firstCharacter, finalCharacter);
    array.push(word);
  }
  let newSentance = array.toString().replaceAll(",", "_");
  const snakeCase = newSentance.toLowerCase();
  console.log(array, snakeCase);
